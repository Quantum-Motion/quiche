...
int main() {
    ...
    double phasePrecision = 0.1 * t / (2 * const_PI);

    int numAncillas = -std::log2(phasePrecision) + std::ceil(std::log2(2 + 1 / (2 * (1 - successProb))));
    int numDataQubits = 4;

    // Data register: [0, numDataQubits)
    // Ancilla register: [numDataQubits, numDataQubits+numAncillas)
    std::vector<int> ancillas(numAncillas);
    std::iota(ancillas.begin(), ancillas.end(), numDataQubits);
